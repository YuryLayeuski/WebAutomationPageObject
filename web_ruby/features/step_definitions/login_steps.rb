...
Then(/^I will see invalid login or password error message$/) do
  expect(usps.login_page.invalid_login_password_error.text).to be == "Invalid format for password."
end
...
Then(/^I will see password length error message$/) do
  expect(usps.login_page.password_length_error.text).to be == "Your Password must be at least 8 characters long."
end
